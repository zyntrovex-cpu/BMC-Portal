...
$st = $db->prepare(
    'SELECT r.*, u.name AS student_name, u2.name AS reporter_name,
            s.roll_no, s.id AS sid, s.father_name, s.dob, c.name AS class_name
...
if ($user['role'] === 'teacher') {
    ...
    if ($teacher) {
        $chk = $db->prepare(
            'SELECT 1 FROM class_subjects cs JOIN students st ON st.class_id=cs.class_id
...
// Migrate old format to new subjects array
...
.stu-row .lbl { color:#64748b; min-width:85px; }
...
<div class="stu-grid">
  <div class="stu-row"><span class="lbl">Name:</span><strong><?= h($rec['student_name']) ?></strong></div>
  <div class="stu-row"><span class="lbl">Father's Name:</span><strong><?= h($rec['father_name']?:'—') ?></strong></div>
  <div class="stu-row"><span class="lbl">Class &amp; Section:</span><strong><?= h($rec['class_name']??'—') ?></strong></div>
  <div class="stu-row"><span class="lbl">Attendance:</span><strong><?= h($fdBasic['attendance']?:'—') ?></strong></div>
  <div class="stu-row"><span class="lbl">GR No:</span><strong><?= h($rec['roll_no']?:'—') ?></strong></div>
  <div class="stu-row"><span class="lbl">Date of Birth:</span><strong><?= h($dobFmt) ?></strong></div>
  <div class="stu-row"><span class="lbl">Term:</span><strong><?= h($rec['term']) ?></strong></div>
  <div class="stu-row"><span class="lbl">Date of Issue:</span><strong><?= h($issueFmt) ?></strong></div>
</div>
...
<?php foreach ($subjects as $subj):
  ...
  // Light tinted thead / area-header backgrounds via inline style
?>
<div class="sec-title" style="background:<?= h($color) ?>"><?= h($subj['title'] ?? '') ?></div>
<table class="pr-tbl">
  ...
  <tr style="background:<?= h($color) ?>15">
    <td colspan="3" style="font-weight:600;font-size:8pt;color:<?= h($color) ?>;padding:2px 8px">
      <?= h($area['title'] ?? '') ?>
    </td>
  </tr>
  ...
  <tr>
    ...
    <td class="ind-cell"><?= pdfIndicator($ind['value'] ?? '') ?></td>
  </tr>
  <?php endforeach; ?>
  <?php endforeach; ?>
  </tbody>
