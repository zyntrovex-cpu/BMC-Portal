...
$tableExists = false;
try {
    $db->query('SELECT 1 FROM speech_therapy_reports LIMIT 1');
    $tableExists = true;
} catch (Exception $e) {}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $tableExists) {
    ...
    if (in_array($action, ['create', 'update']) && $studentId && $monthDate) {
        ...
        if ($action === 'update') {
            ...
            $db->prepare('UPDATE speech_therapy_reports SET therapist_notes=?,progress_summary=?,goals_next_month=?,recorded_by=? WHERE id=?')
               ->execute([$notes ?: null, $progress ?: null, $goals ?: null, $user['id'], $id]);
            logActivity($user['id'], 'stherapy_update', "Updated speech therapy report #$id");
            setFlash('success', 'Report updated.');
        } else {
            $db->prepare('INSERT INTO speech_therapy_reports (student_id,month,therapist_notes,progress_summary,goals_next_month,recorded_by) VALUES (?,?,?,?,?,?)
            ...
            logActivity($user['id'], 'stherapy_create', "Saved speech therapy report for student #$studentId ($month)");
    ...
    if ($action === 'delete') {
        ...
        if ($id) {
            ...
            logActivity($user['id'], 'stherapy_delete', "Deleted speech therapy report #$id");
...
<div class="main-area">
<?php topbar('Speech Therapy Reports', $user); ?>
...
<div class="d-flex align-items-center gap-3 mb-3 p-3" style="background:linear-gradient(90deg,#ecfeff,#f0fdf4);border-radius:10px;border:1px solid #a5f3fc;">
  ...
  <div>
    <div style="font-size:.72rem;font-weight:700;color:#0891b2;letter-spacing:.8px;text-transform:uppercase">Speech Therapy — Monthly Reports</div>
...
<div class="row g-3">
      ...
      <div style="padding:16px">
        <form method="POST">
          ...
          <div class="mb-2">
            ...
            <input type="month" name="month" class="form-control form-control-sm" required value="<?= date('Y-m') ?>">
          </div>
          <?php endif; ?>
          <div class="mb-2">
            ...
            <textarea name="therapist_notes" class="form-control form-control-sm" rows="3"
                      placeholder="Sessions, techniques used…"><?= h($editReport['therapist_notes'] ?? '') ?></textarea>
          </div>
          <div class="mb-2">
            ...
            <textarea name="progress_summary" class="form-control form-control-sm" rows="3"
                      placeholder="Speech milestones achieved…"><?= h($editReport['progress_summary'] ?? '') ?></textarea>
          </div>
          <div class="mb-3">
  ...
  <div class="col-lg-7">
    ...
    <div class="sec-card">
      <div class="sec-card-header">
        <i class="fas fa-comment-medical me-2"></i>Reports — <?= h($curStudent['name']) ?>
