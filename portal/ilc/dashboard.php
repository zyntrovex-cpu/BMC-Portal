...
<div class="portal-banner mb-4" style="background:linear-gradient(135deg,#0c4a6e,#0891b2);padding:0;overflow:hidden;position:relative;">
  ...
  <div class="d-flex align-items-center gap-4 p-3 ps-4">
    <?php
      $_av = _avatarHtml($user['id'], _initials($user['name']), 64);
      if (str_starts_with($_av, '<img')):
    ?><div style="width:64px;height:64px;border-radius:50%;overflow:hidden;flex-shrink:0;border:3px solid rgba(255,255,255,.6);box-shadow:0 4px 16px rgba(0,0,0,.25)"><?= $_av ?></div><?php
      else: ?><div style="width:64px;height:64px;border-radius:50%;background:rgba(255,255,255,.22);display:flex;align-items:center;justify-content:center;font-size:1.5rem;font-weight:700;color:#fff;flex-shrink:0;border:3px solid rgba(255,255,255,.4);box-shadow:0 4px 16px rgba(0,0,0,.25)"><?= $_av ?></div><?php
      endif; ?>
    <div>
