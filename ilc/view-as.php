...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['target_id'])) {
    ...
    // Students must be in an ILC class; teachers must have is_ilc=1
    $st = $db->prepare(
        'SELECT u.id, u.user_id, u.name, u.email, u.role, u.status
         ...
         WHERE u.id = ?
           ...
           AND (
               (u.role = "student" AND c.is_ilc = 1)
               OR
               (u.role = "teacher" AND t.is_ilc  = 1)
           )'
    );
    $st->execute([$targetId]);
    $target = $st->fetch();
...
<div class="alert alert-info d-flex gap-2 align-items-start" style="font-size:.86rem;border-radius:8px">
  ...
  <div><strong>ILC Portal Preview</strong> — Click <em>View Portal</em> to see a student or teacher's portal exactly as they see it. A yellow banner will appear — click <em>Exit Preview</em> to return here. Only ILC students and teachers are shown.</div>
</div>
