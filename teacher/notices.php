...
<form method="GET" class="d-flex gap-2 mb-3 flex-wrap">
  <input type="text" name="q" class="form-control form-control-sm" style="max-width:260px" placeholder="Search..." value="<?= h($q) ?>">
...
<?php else: ?>
  ...
  <div class="sec-card mb-3 <?= $n['pinned']?'border-warning':'' ?>">
    <div class="sec-card-header d-flex justify-content-between align-items-start flex-wrap gap-2">
      <div><?php if($n['pinned']): ?><i class="fas fa-thumbtack text-warning me-1"></i><?php endif; ?><strong><?= h($n['title']) ?></strong></div>
      <div class="d-flex gap-1"><span class="badge bg-<?= $catClass ?>"><?= h($n['category']) ?></span><span class="badge bg-<?= $priorityClass ?>"><?= h($n['priority']) ?></span></div>
    </div>
